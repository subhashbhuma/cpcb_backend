...
function menu_click() {
    ...
    for (var i = 0; i < pc_link_click.length; i++) {
        pc_link_click[i].addEventListener("click", function (event) {
            ...
            var targetElement = event.currentTarget; // Use currentTarget to always get the LI or anchor
            var submenu = targetElement.children[1];
            ...
            if (targetElement.classList.contains("pc-trigger")) {
                ...
                var siblingTriggers = targetElement.parentNode.querySelectorAll(":scope > li.pc-trigger");
                for (var t = 0; t < siblingTriggers.length; t++) {
                    var c = siblingTriggers[t];
...
for (var l = 0, len = elem.length; l < len; l++) {
    var link = elem[l];
    if (link.href === pageUrl && link.getAttribute("href") != "") {
        var parentLi = link.parentNode;
        parentLi.classList.add("active");

        var p1 = parentLi.parentNode; // .pc-submenu or .pc-navbar
        if (p1 && p1.classList.contains("pc-submenu")) {
            var p2 = p1.parentNode; // .pc-hasmenu
            if (p2) {
                p2.classList.add("pc-trigger");
                p2.classList.add("active");
                p1.style.display = "block";
                
                var p3 = p2.parentNode; // .pc-submenu or .pc-navbar
                if (p3 && p3.classList.contains("pc-submenu")) {
                    var p4 = p3.parentNode; // .pc-hasmenu
                    if (p4) {
                        p4.classList.add("pc-trigger");
