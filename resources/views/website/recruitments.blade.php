                        ...
                        <div>
                            <div class="table-responsive">
                                <table class="table table-hover mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        @if ($reqruitments->count() > 0)
                                        @foreach ($reqruitments as $reqruitment)
                                        <tr>
                                            <td class="px-4 py-3">
                                                <div>
                                                    @if($reqruitment->published_date > now()->subDays(15))
                                                    <div class="mt-1">
                                                        <span class="badge bg-primary text-white rounded-pill">New</span>
                                                    </div>
                                                    @endif
                                                    {{ getLocalizedDataFromObj($reqruitment, 'title') }}
                                                    <div class="text-muted mb-0 small">
                                                        {!! getLocalizedDataFromObj($reqruitment, 'description') !!}
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-4 py-3 text-nowrap text-muted">
                                                @if($reqruitment->published_date)
                                                <i class="fas fa-calendar-alt text-primary me-2" aria-hidden="true"></i>
                                                {{ $reqruitment->published_date->format('d M Y') }}
                                                @endif
                                            </td>
                                            <td class="px-4 py-3 text-nowrap">
                                                <div class="d-flex gap-2">
                                                    <a
                                                        href="{{ getLocalizedDataFromObj($reqruitment, 'file_url') }}"
                            ...
                            <div class="d-flex justify-content-end align-items-center p-4 border-top">
                                <div class="pagination-wrapper">
                                    {{ $reqruitments->links('pagination::bootstrap-5') }}
                                </div>
                            </div>
                        </div>
                    </div>
