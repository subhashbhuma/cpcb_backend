...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('gallery-event.create');
    @endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="gallery-event-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            ...
            const table = $('#gallery-event-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('gallery-event.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * DELETE EVENT
             * =============================== */
            $(document).on('click', '.delete-gallery-event', function () {
                let id = $(this).data('id');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('gallery-event.destroy', ':id') }}".replace(
                                ':id', id),
                            type: "DELETE",
                            headers: {
