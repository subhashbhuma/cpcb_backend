                            ...
                            <p class="mb-0">
                                @if ($quickLink->link)
                                    <a href="{{ $quickLink->link }}" target="_blank"
                                        rel="noopener noreferrer">{{ $quickLink->link }}</a>
                                @else
                                    —
                                @endif
                            </p>
            ...
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            @if ($quickLink->is_approved == 1)
                                <span class="badge bg-success">Approved</span>
                            @elseif ($quickLink->is_approved == 2)
            ...
            $(document).ready(function () {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        ...
                        $.ajax({
                            url: "{{ route('quick-links.approve', $quickLink->id) }}",
                            type: "POST",
                            data: formData,
                            processData: false,
                            contentType: false,
            ...
            $(document).ready(function () {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        let formData = new FormData(form);
                        $.ajax({
                            url: "{{ route('quick-links.publish', $quickLink->id) }}",
                            type: "POST",
                            data: formData,
                            processData: false,
                            contentType: false,
