...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('quick-links.create');
    @endphp
    @can('add slider')
        @php
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="quick-links-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#quick-links-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('quick-links.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-quick-link', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('quick-links.destroy', ':id') }}".replace(':id', roleId),
                            type: "DELETE",
                            headers: {
