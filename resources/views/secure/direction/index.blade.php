...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('direction.create');
    @endphp
    @can('add direction')
        @php
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        ...
                        <table id="directions-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#directions-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('direction.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
            ...
            $('#status_filter').on('change', function () {
                $('#directions-datatable').DataTable().draw();
            });
            ...
            $(document).on('click', '.delete-direction', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('direction.destroy', ':id') }}".replace(':id', roleId),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#directions-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
