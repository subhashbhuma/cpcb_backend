                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Quality Zone:</strong></label>
                            <p class="mb-0">{{ $record->qualityZone->title ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>For Date:</strong></label>
                            <p class="mb-0">{{ $record->for_date ?? '—' }}</p>
                        </div>
                        ...
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">
                                @if($record->file_name)
                                    <a href="{{ generate_file_view_path_for_backend($record->file_en_url) }}" target="_blank">View
                                        Document</a>
                                @else
                        ...
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">
                                @if($record->file_name_hi)
                                    <a href="{{ generate_file_view_path_for_backend($record->file_hi_url) }}" target="_blank">View
                                        Document</a>
                                @else
                                    <span class="text-muted">No file uploaded.</span>
                                @endif
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('agra-air-qualities.approve', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('agra-air-qualities.publish', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
