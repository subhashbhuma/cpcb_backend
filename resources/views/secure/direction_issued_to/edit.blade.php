...
@section('content')
    <!-- [ Page Header ] start -->
    <x-page-header title="Edit Direction Issued To" :backButton="true" />
    <!-- [ Page Header ] end -->
                            ...
                            <div class="col-md-12 col-12 mb-3">
                                ...
                                <select name="direction_act_type_id" class="form-control">
                                    ...
                                    @foreach($actTypes as $type)
                                        <option value="{{ $type->id }}" {{ $directionIssuedTo->direction_act_type_id == $type->id ? 'selected' : '' }}>{{ $type->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 col-12 mb-3">
                                ...
                                <input type="text" name="title_hi" id="title_hi" class="form-control"
                                    value="{{ $directionIssuedTo->title_hi }}" />
                            </div>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $("#directionIssuedToForm").validate({
                rules: {
                    title: {
                        required: true
                    },
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        url: "{{ route('direction_issued_to.update', $directionIssuedTo->id) }}",
                        method: "POST",
                        data: formData,
                        contentType: false,
                        processData: false,
