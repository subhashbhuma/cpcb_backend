...
@section('content')
    @php
        $addRoute = route('direction_act_type.create');
    @endphp

    <x-page-header title="Direction Act Type List"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="direction-act-type-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#direction-act-type-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('direction_act_type.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                $('#direction-act-type-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
