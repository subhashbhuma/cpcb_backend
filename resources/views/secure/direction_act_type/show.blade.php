    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">{{ $directionActType->title ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Title (Hindi):</strong></label>
                            <p class="mb-0">{{ $directionActType->title_hi ?? '—' }}</p>
                        </div>
            ...
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            @if ($directionActType->is_approved == 1)
                                <span class="badge bg-success">Approved</span>
                            @elseif ($directionActType->is_approved == 2)
        ...
        : '<span class="badge bg-warning text-dark">Not Published</span>' !!}
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionActType->remarks ? $directionActType->remarks : '—' !!}
                        </p>
                    </div>
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionActType->publish_remark ? $directionActType->publish_remark : '—' !!}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                @can('approve direction act type')
                ...
                @can('publish direction act type')
                    <div class="col-md-6 col-12 mb-3">
                        <div class="card h-100">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction_act_type.approve', $directionActType->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
                                    window.location.href = "{{ route('direction_act_type.index') }}";
                                });
                            },
                            error: function (xhr) {
                                if (xhr.status === 422) {
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction_act_type.publish', $directionActType->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
