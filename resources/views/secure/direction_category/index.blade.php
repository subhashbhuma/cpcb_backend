...
@section('content')
    @php
        $addRoute = route('direction_category.create');
    @endphp

    <x-page-header title="Direction Category List"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="direction-category-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No</th>
                                    <th>Act Type</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#direction-category-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('direction_category.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                $('#direction-category-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
