...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('epr-portals.create');
    @endphp
    @can('add epr portal')
        @php
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="epr-portals-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#epr-portals-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('epr-portals.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-epr-portal', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('epr-portals.destroy', ':id') }}".replace(':id', roleId),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#epr-portals-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
