...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('photo-gallery.create');
    @endphp
    @can('add user')
        @php
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="photo-gallery-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
    ...
    <script @cspNonce>
        $(document).ready(function () {
            $('#photo-gallery-datatable').DataTable({
                ...
                ordering: false,
                ajax: {
                    url: "{{ route('photo-gallery.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-photo-gallery', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('photo-gallery.destroy', ':id') }}".replace(
                                ':id', roleId),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#photo-gallery-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
