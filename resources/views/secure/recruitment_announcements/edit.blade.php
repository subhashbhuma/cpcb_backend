        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form id="recruitmentAnnouncementEditForm" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-4 border-bottom pb-3">
                            <div class="col-md-6 col-12 mb-3">
                                ...
                                <select name="type" class="form-control" required>
                                    <!-- <option value="result" {{ $announcement->type == 'result' ? 'selected' : '' }}>Result
                                                            </option> -->
                            ...
                            <div class="col-md-6 col-12 mb-3">
                                ...
                                <select name="job_id" id="job_id" class="form-control select2" required>
                                    ...
                                    @foreach($jobs as $job)
                                        <option value="{{ $job->id }}" {{ $announcement->jobPost->job_id == $job->id ? 'selected' : '' }}>{{ $job->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    <div id="postChecklist" class="p-2" style="max-height: 250px; overflow-y: auto;">
                                        @foreach($jobPosts as $post)
                                            <div class="checklist-item form-check d-flex align-items-start gap-2">
                                                <input class="form-check-input post-checkpoint mt-1" type="checkbox"
                                                    name="job_post_id[]" value="{{ $post->id }}" id="post_{{ $post->id }}" {{ $announcement->job_post_id == $post->id ? 'checked' : '' }}>
                                                <label class="form-check-label cursor-pointer flex-grow-1"
                                                    for="post_{{ $post->id }}">
                                                    {{ $post->title }}
                            ...
                            <div class="col-md-6 col-12 mb-3">
                                ...
                                @if ($announcement->file_name)
                                    <div>
                                        Current File: <a
                                            href="{{ generate_file_view_path_for_backend($announcement->file_url) }}"
                            ...
                            <div class="col-md-6 col-12 mb-3">
                                ...
                                @if ($announcement->file_name_hi)
                                    <div>
                                        Current File: <a
                                            href="{{ generate_file_view_path_for_backend($announcement->file_url_hi) }}"
                                            target='_BLANK'>View
                                            Document</a>
                                    </div>
                                @endif
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            ...
            $("#recruitmentAnnouncementEditForm").validate({
                rules: {
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        url: "{{ route('recruitment-announcements.update', $announcement->id) }}",
                        method: "POST",
                        data: formData,
                        contentType: false,
                        processData: false,
