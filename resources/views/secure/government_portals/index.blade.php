...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('government-portals.create');
    @endphp
    @can('add slider')
        @php
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="government-portals-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>File</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#government-portals-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('government-portals.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-government-portal', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('government-portals.destroy', ':id') }}".replace(':id', roleId),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#government-portals-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
