    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">{{ $directionSubject->directionActType->title ?? '—' }}</p>
                        </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="col-md-6 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">{{ $directionSubject->title ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Title (Hindi):</strong></label>
                            <p class="mb-0">{{ $directionSubject->title_hi ?? '—' }}</p>
                        </div>
            ...
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            @if ($directionSubject->is_approved == 1)
                                <span class="badge bg-success">Approved</span>
                            @elseif ($directionSubject->is_approved == 2)
        ...
        : '<span class="badge bg-warning text-dark">Not Published</span>' !!}
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionSubject->remarks ? $directionSubject->remarks : '—' !!}
                        </p>
                    </div>
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionSubject->publish_remark ? $directionSubject->publish_remark : '—' !!}
                        </p>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction_subject.approve', $directionSubject->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
                                    window.location.href = "{{ route('direction_subject.index') }}";
                                });
                            },
                            error: function (xhr) {
                                if (xhr.status === 422) {
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction_subject.publish', $directionSubject->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
