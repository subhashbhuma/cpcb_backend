...
@section('content')
    @php
        $addRoute = route('direction_subject.create');
    @endphp

    <x-page-header title="Direction Subject List"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="direction-subject-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No</th>
                                    <th>Act Type</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#direction-subject-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('direction_subject.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                $('#direction-subject-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
