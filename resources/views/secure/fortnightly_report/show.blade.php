...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
                        ...
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">
                                @if($record->file_name_hi)
                                    <a href="{{ generate_file_view_path_for_backend($record->file_url_hi) }}" target="_blank">View
                                        Document</a>
                                @else
                                    <span class="text-muted">No file uploaded.</span>
                                @endif
            ...
            <div class="row">
                @can('approve fortnightly report')
                ...
                @can('publish fortnightly report')
                    <div class="col-md-6 col-12 mb-3">
                        <div class="card h-100">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('fortnightly-reports.approve', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('fortnightly-reports.publish', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
