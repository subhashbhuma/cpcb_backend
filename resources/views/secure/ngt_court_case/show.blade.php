    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">{{ $record->title_hi ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>QA Number (English):</strong></label>
                            <p class="mb-0">{{ $record->qa_number ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>QA Number (Hindi):</strong></label>
                            <p class="mb-0">{{ $record->qa_number_hi ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Type (English):</strong></label>
                            <p class="mb-0">{{ $record->type ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Type (Hindi):</strong></label>
                            <p class="mb-0">{{ $record->type_hi ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Publish Date:</strong></label>
                            <p class="mb-0">{{ $record->publish_date ? date('d-m-Y', strtotime($record->publish_date)) : '—' }}
                            </p>
                        </div>
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">
                                @if($record->file_name)
                                    <a href="{{ generate_file_view_path_for_backend($record->file_url) }}" target="_blank">View
                                        Document</a>
                                @else
        ...
        : '<span class="badge bg-warning text-dark">Not Published</span>' !!}
                    ...
                    <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $record->publish_remark ? $record->publish_remark : '—' !!}
                        </p>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('ngt-court-cases.approve', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('ngt-court-cases.publish', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
