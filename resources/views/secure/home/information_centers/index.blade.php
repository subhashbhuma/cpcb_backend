...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('information-centers.create');
    @endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="information-centers-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            const table = $('#information-centers-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('information-centers.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
            ...
            /* ===============================
            * Implementation of delete record
            * =============================== */
            $(document).on('click', '.delete-information-center', function () {
                let id = $(this).data('id');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('information-centers.destroy', ':id') }}".replace(':id', id),
                            type: "DELETE",
                            headers: {
