...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('publication_category.create');
    @endphp
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#publication_category-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                responsive: {
                ...
                autoWidth: false,
                ajax: {
                    url: "{{ route('publication_category.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-publication_category', function () {
                let Id = $(this).data('id');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('publication_category.destroy', ':id') }}"
                                .replace(':id', Id),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#publication_category-datatable').DataTable().ajax
                                    .reload();
                            },
                            error: function (xhr) {
                                hideLoader();
