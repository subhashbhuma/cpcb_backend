...
@section('content')
    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Series Name (English):</strong></label>
                            <p class="mb-0">{{ $publication_category->code ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Series Name (Hindi):</strong></label>
                            <p class="mb-0">{{ $publication_category->code_hi ?? '—' }}</p>
                        </div>
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Series Title (English):</strong></label>
                            <p class="mb-0">{{ $publication_category->title ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Series Title (Hindi):</strong></label>
                            <p class="mb-0">{{ $publication_category->title_hi ?? '—' }}</p>
                        </div>


                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Remarks:</strong></label>
                            <p class="mb-0">{!! $publication_category->remarks ?? '—' !!}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Publish Remark:</strong></label>
                            <p class="mb-0">{!! $publication_category->publish_remark ?? '—' !!}</p>
                        </div>

                        <div class="col-md-6 col-12 mb-3 card py-2 bg-light">
                            <label class="form-label">Created Date:</label>
                            <p class="mb-0">
                                {{ $publication_category?->created_at?->format('d M Y') }}
                            </p>
                        </div>
                        <div class="col-md-6 col-12 mb-3 card py-2 bg-light">
                            <label class="form-label">Created By</label>
                            <p class="mb-0">
                                {{ $publication_category->created_by_user ? $publication_category->created_by_user->name : 'N/A' }}
                            </p>
                        </div>
                        ...
                        <div class="col-md-6 col-12 mb-3 card py-2 bg-light">
                            ...
                            <p class="mb-0">
                                {{ $publication_category?->updated_at?->format('d M Y') }}
                            </p>
                        </div>
                        <div class="col-md-6 col-12 mb-3 card py-2 bg-light">
                            <label class="form-label">Updated By</label>
                            <p class="mb-0">
                                {{ $publication_category->updated_by_user ? $publication_category->updated_by_user->name : 'N/A' }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            @if ($publication_category->is_approved == 1)
                                <span class="badge bg-success">Approved</span>
                            @elseif ($publication_category->is_approved == 2)
                    ...
                    <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $publication_category->is_published
        ...
        : '<span class="badge bg-warning text-dark">Not Published</span>' !!}
            ...
            <div class="row">
                @can('approve publication category')
                ...
                @can('publish publication category')
                    <div class="col-md-6 col-12 mb-3">
                        <div class="card h-100">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('publication_category.approve', $publication_category->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
                                    window.location.href = "{{ route('publication_category.index') }}";
                                });
                            },
                            error: function (xhr) {
                                if (xhr.status === 422) {
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('publication_category.publish', $publication_category->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
