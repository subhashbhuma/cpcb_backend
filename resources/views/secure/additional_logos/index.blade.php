...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('additional-logos.create');
    @endphp
    @can('add slider')
        @php
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="additional-logos-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>File</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#additional-logos-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('additional-logos.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-additional-logo', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('additional-logos.destroy', ':id') }}".replace(':id', roleId),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#additional-logos-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
