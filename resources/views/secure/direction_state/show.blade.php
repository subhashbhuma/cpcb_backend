    ...
    <div class="row">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">{{ $directionState->title ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Title (Hindi):</strong></label>
                            <p class="mb-0">{{ $directionState->title_hi ?? '—' }}</p>
                        </div>
            ...
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            @if ($directionState->is_approved == 1)
                                <span class="badge bg-success">Approved</span>
                            @elseif ($directionState->is_approved == 2)
        ...
        : '<span class="badge bg-warning text-dark">Not Published</span>' !!}
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionState->remarks ? $directionState->remarks : '—' !!}
                        </p>
                    </div>
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionState->publish_remark ? $directionState->publish_remark : '—' !!}
                        </p>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction_state.approve', $directionState->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
                                    window.location.href = "{{ route('direction_state.index') }}";
                                });
                            },
                            error: function (xhr) {
                                if (xhr.status === 422) {
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction_state.publish', $directionState->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
