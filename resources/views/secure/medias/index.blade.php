        ...
        <div class="col-12">
                    ...
                    @foreach ($medias as $media)
                        <div class="col-md-3 col-sm-6 col-12 mb-3">
                            <div class="card  shadow">
                                <a href="{{ asset('storage/' . Config::get('file_paths')['MEDIA_IMAGE_PATH'] . '/' . $media->file_name) }}"
                                    target="_blank" class="cursor-pointer">
                                    @if ($media->mime_type == 'video/mp4')
                                        <img src="{{ asset('assets/images/video.png') }}"
                                            alt="{{ $media->alt_text ?? $media->alt_text_hi }}"
                                            title="{{ $media->alt_text ?? $media->alt_text_hi }}"
                                            class="card-img-top img-thumbnail media-preview-img">
                                    @elseif (preg_match('/^image\//', $media->mime_type))
                                        <img src="{{ $media->media_public_url }}" alt="{{ $media->alt_text ?? $media->alt_text_hi }}"
                                            title="{{ $media->alt_text ?? $media->alt_text_hi }}"
                                            class="card-img-top img-thumbnail media-preview-img">
                                    @else
                                ...
                                <div class="card-body py-2 mb-0">
                                    <a href="{{ $media->media_public_url }}" target="_blank" class="cursor-pointer">
                                        <!-- text-truncate -->
                                        <h5 class="card-title ">{{ $media->original_name }}</h5>
                                    </a>
                                    <!-- Size -->
                                    <p class="text-dark mb-0"><strong>Size:</strong> {{ formatBytes($media->size) }}</p>
                                    <!-- Mime type -->
                                    <p class="text-dark mb-0 text-truncate" title="{{ $media->mime_type }}"><strong>Mime
                                            type:</strong> {{ $media->mime_type }}</p>
                                    <!-- Url -->
                                    <hr class="my-2">
                                    <p class="text-dark mb-2">
                                        <strong>Public Url:</strong>
                                        <span class="copyPublicUrl cursor-pointer"
                                            title="Click to copy">{{ $media->media_public_url }}</span>
                                    </p>
                                </div>
            ...
            @else
                <div class="alert alert-danger">
                    <strong>Ooops!</strong> No media available
                </div>
            @endif
        </div>
    </div>
