    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-12 card py-2 bg-light mb-3">
                            ...
                            <div>
                                @if ($cpcbPortal->file_name)
                                    <a href="{{ $cpcbPortal->file_name_full_path }}" target="__blank"><i
                        ...
                        <div class="col-md-6 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">{{ $cpcbPortal->title ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Title (Hindi):</strong></label>
                            <p class="mb-0">{{ $cpcbPortal->title_hi ?? '—' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            @if ($cpcbPortal->is_approved == 1)
                                <span class="badge bg-success">Approved</span>
                            @elseif ($cpcbPortal->is_approved == 2)
        ...
        : '<span class="badge bg-warning text-dark">Not Published</span>' !!}
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {{ $cpcbPortal->remarks ? $cpcbPortal->remarks : 'No remarks available' }}
                        </p>
                    </div>
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {{ $cpcbPortal->publish_remark ?: 'No publish remarks available' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        ...
                        $.ajax({
                            url: "{{ route('cpcb-portals.approve', $cpcbPortal->id) }}",
                            type: "POST",
                            data: formData,
                            processData: false,
                            contentType: false,
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success")
                                    .then(() => window.location.href = "{{ route('cpcb-portals.index') }}");
                            },
                            error: function (xhr) {
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        ...
                        $.ajax({
                            url: "{{ route('cpcb-portals.publish', $cpcbPortal->id) }}",
                            type: "POST",
                            data: formData,
                            processData: false,
                            contentType: false,
