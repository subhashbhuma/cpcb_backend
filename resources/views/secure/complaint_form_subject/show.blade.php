...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="row">
                @can('approve complaint form subject')
                ...
                @can('publish complaint form subject')
                    <div class="col-md-6 col-12 mb-3">
                        <div class="card h-100">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('complaint_form_subject.approve', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('complaint_form_subject.publish', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
