...
@section('content')
    @php
        $addRoute = route('complaint_form_subject.create');
    @endphp

    <x-page-header title="Complaint Form Subject List"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="complaint-form-subject-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#complaint-form-subject-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('complaint_form_subject.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                $('#complaint-form-subject-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
