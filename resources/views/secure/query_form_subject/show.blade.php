...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Subject Title (English):</strong></label>
                            <p class="mb-0">{{ $record->title ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Subject Title (हिंदी):</strong></label>
                            <p class="mb-0">{{ $record->title_hi ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Division:</strong></label>
                            <p class="mb-0">{{ $record->division?->title ?? '—' }} /
                                {{ $record->division?->title_hi ?? '—' }}
                            </p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Officer Name (English):</strong></label>
                            <p class="mb-0">{{ $record->name ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Officer Name (हिंदी):</strong></label>
                            <p class="mb-0">{{ $record->name_hi ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Officer Email ID:</strong></label>
                            <p class="mb-0">{{ $record->email_id ?? '—' }}</p>
                        </div>
            ...
            <div class="row">
                @can('approve query form subject')
                ...
                @can('publish query form subject')
                    <div class="col-md-6 col-12 mb-3">
                        <div class="card h-100">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('query_form_subject.approve', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('query_form_subject.publish', $record->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
