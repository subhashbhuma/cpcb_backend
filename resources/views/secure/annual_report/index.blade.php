...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('annual_report.create');
    @endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        ...
                        <table id="annual_report-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#annual_report-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('annual_report.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
            ...
            $('#status_filter').on('change', function () {
                $('#annual_report-datatable').DataTable().draw();
            });

            /**
             * Delete record
             */
            $(document).on('click', '.delete-annual_report', function () {
                let Id = $(this).data('id');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('annual_report.destroy', ':id') }}".replace(':id', Id),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#annual_report-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
