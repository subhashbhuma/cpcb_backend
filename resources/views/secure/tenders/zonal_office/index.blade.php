...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('zonal_office.create');
    @endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="zonal-office-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Title (English)</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#zonal-office-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                responsive: {
                ...
                autoWidth: false,
                ajax: {
                    url: "{{ route('zonal_office.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-zonal-office', function () {
                let Id = $(this).data('id');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('zonal_office.destroy', ':id') }}"
                                .replace(':id', Id),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#zonal-office-datatable').DataTable().ajax
                                    .reload();
                            },
                            error: function (xhr) {
                                hideLoader();
