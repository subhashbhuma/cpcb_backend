...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('tender_category.create');
    @endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="tender_category-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Title (English)</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#tender_category-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                responsive: {
                ...
                autoWidth: false,
                ajax: {
                    url: "{{ route('tender_category.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
             ...
             * Delete record
             */
            $(document).on('click', '.delete-tender_category', function () {
                let Id = $(this).data('id');
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('tender_category.destroy', ':id') }}"
                                .replace(':id', Id),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#tender_category-datatable').DataTable().ajax
                                    .reload();
                            },
                            error: function (xhr) {
                                hideLoader();
