...
@section('content')
    @php
        $addRoute = route('page_category.create');
    @endphp

    <x-page-header title="Page Category List"
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="page-category-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#page-category-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('page_category.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                $('#page-category-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
