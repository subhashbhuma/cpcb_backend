...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('designation.create');
    @endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="designation-datatable" class="table table-striped table-bordered w-100 no-wrap">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            ...
            const table = $('#designation-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('designation.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
            ...
            $(document).on('click', '.delete-designation', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('designation.destroy', ':id') }}".replace(':id',
                                id),
                            type: "DELETE",
                            headers: {
