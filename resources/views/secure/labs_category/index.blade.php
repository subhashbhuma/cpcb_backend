...
@section('content')
    @php
        $addRoute = route('labs_category.create');
    @endphp
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        <table id="labs-category-datatable" class="table table-striped table-bordered w-100">
                            <thead>
                                <tr>
                                    <th>Sl. No</th>
                                    <th>Title</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#labs-category-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                ajax: {
                    url: "{{ route('labs_category.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
            ...
            $(document).on('click', '.delete-labs_category', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('labs_category.destroy', ':id') }}".replace(':id', roleId),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#labs-category-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
