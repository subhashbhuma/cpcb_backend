                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            ...
                            <p class="mb-0">₹ {{ $publication->price ?? '—' }}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Published Date:</strong></label>
                            <p class="mb-0">
                                {{ $publication->published_date ? \Carbon\Carbon::parse($publication->published_date)->format('d-m-Y') : '—' }}
                            </p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Remarks:</strong></label>
                            <p class="mb-0">{!! $publication->remarks ?? '—' !!}</p>
                        </div>

                        <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                            <label class="form-label"><strong>Publish Remark:</strong></label>
                            <p class="mb-0">{!! $publication->publish_remark ?? '—' !!}</p>
                        </div>
                ...
                <div class="row">
                    <div class="col-md-6 col-12 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            ...
                            @elseif ($publication->is_approved == 2)
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('publication.approve', $publication->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('publication.publish', $publication->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
