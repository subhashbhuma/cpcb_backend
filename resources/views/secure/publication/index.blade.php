...
@section('content')
    <!-- [ Page Header ] start -->
    @php
        $addRoute = route('publication.create');
    @endphp
    @can('add publication')
        @php
            ...
            <div class="card">
                <div class="card-body">
                    <div class="dt-responsive">
                        ...
                        <table id="publication-datatable" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    <th>Sl. No.</th>
                                    <th>Category</th>
...
@section('pages-scripts')
    <script @cspNonce>
        $(document).ready(function () {
            $('#publication-datatable').DataTable({
                processing: true,
                serverSide: true,
                ordering: false,
                scrollX: true,
                ajax: {
                    url: "{{ route('publication.fetch-for-datatable') }}",
                    type: "POST",
                    data: function (d) {
            ...
            $('#status_filter').on('change', function () {
                $('#publication-datatable').DataTable().draw();
            });
            ...
            $(document).on('click', '.delete-publication', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('publication.destroy', ':id') }}".replace(':id', Id),
                            type: "DELETE",
                            headers: {
                            ...
                            success: function (response) {
                                ...
                                $('#publication-datatable').DataTable().ajax.reload();
                            },
                            error: function (xhr) {
                                hideLoader();
