...
@section('content')
    <!-- [ Page Header ] start -->
    <x-page-header title="{{ $pageTitle ?? 'View Direction Type' }}" :backButton="true" />
    <!-- [ Page Header ] end -->
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 col-12 mb-3 card py-2 bg-light">
                            <label class="form-label">Act Type:</label>
                            <p class="mb-0">{{ $directionType->directionActType->title ?? '—' }}</p>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-6 col-12 mb-3 card py-2 bg-light">
                            <label class="form-label">Title (English):</label>
                            <p class="mb-0">{{ $directionType->title }}</p>
                        </div>

                        <div class="col-md-6 col-12 mb-3 card py-2 bg-light">
                            <label class="form-label">Title (हिंदी):</label>
                            <p class="mb-0">{{ $directionType->title_hi }}</p>
                        </div>
                    </div>
                </div> <!-- card-body -->
            </div> <!-- card -->
            ...
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            @if ($directionType->is_approved == 1)
                                <span class="badge bg-success">Approved</span>
                            @elseif ($directionType->is_approved == 2)
        ...
        : '<span class="badge bg-warning text-dark">Not Published</span>' !!}
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionType->remarks ? $directionType->remarks : '—' !!}
                        </p>
                    </div>
                    ...
                    <div class="col-md-6 card py-2 bg-light mb-3">
                        ...
                        <p class="mb-0">
                            {!! $directionType->publish_remark ? $directionType->publish_remark : '—' !!}
                        </p>
                    </div>
                </div>
            </div>
        ...
        $(document).ready(function () {
            ...
            if ($("#approveForm").length) {
                $("#approveForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction-types.approve', $directionType->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
                                    window.location.href = "{{ route('direction-types.index') }}";
                                });
                            },
                            error: function (xhr) {
                                if (xhr.status === 422) {
            ...
            if ($("#publishForm").length) {
                $("#publishForm").validate({
                    ...
                    submitHandler: function (form) {
                        $.ajax({
                            url: "{{ route('direction-types.publish', $directionType->id) }}",
                            type: "POST",
                            data: $(form).serialize(),
                            success: function (response) {
                                Swal.fire("Success!", response.message, "success").then(() => {
