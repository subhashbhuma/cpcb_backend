...
class UpdateInformationCenterDetailRequest extends FormRequest
{
    ...
    {
        return [
            'information_center_id' => 'sometimes|required|exists:information_centers,id',
            'type' => 'sometimes|required|in:FILE,URL',
            'url' => 'nullable|max:2048|required_if:type,URL|regex:/^[\p{L}\p{N}\p{M}\s\.\,\-_&\(\)\:\;\'’"\/@#\$%?!\+=\[\]\*।॥–—]*$/u',
            'title' => 'sometimes|required|string|max:255|regex:/^[\p{L}\p{N}\p{M}\s\.\,\-_&\(\)\:\;\'’"\/@#\$%?!\+=\[\]\*।॥–—]*$/u',
            'title_hi' => 'nullable|string|max:255|regex:/^[\p{L}\p{N}\p{M}\s\.\,\-_&\(\)\:\;\'’"\/@#\$%?!\+=\[\]\*।॥–—]*$/u',
            'file_name' => 'nullable|file|mimes:pdf,doc,docx,xls,xlsx,txt|max:10240',
