...
class Portal extends Model
{
    ...
    protected $appends = ['is_approved_desc', 'is_published_desc', 'file_name_full_path'];

    public function getFileNameFullPathAttribute()
    {
        return asset('storage/' . Config::get('file_paths')['CPCB_PORTAL_IMAGE_PATH'] . '/' . $this->file_name);
    }
    ...
    public function getActivitylogOptions(): LogOptions
    {
        return LogOptions::defaults()
            ->logAll()
            ->useLogName('cpcbportal')
            ->logOnlyDirty()
            ->setDescriptionForEvent(fn(string $eventName) => "Cpcb portal model has been {$eventName}");
    }
}
