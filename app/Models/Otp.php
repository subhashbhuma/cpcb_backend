...
class Otp extends Model
{
    ...
    public static function generateOtp($email = null, $mobile = null)
    {
        ...
        $isDummyOtp = config('app.is_dummy_otp', false);
        $isDummyOtp = filter_var($isDummyOtp, FILTER_VALIDATE_BOOLEAN);
        if ($isDummyOtp) {
            $otp = 123456;
        } else {
        ...
        $expiresAt = Carbon::now()->addMinutes((int) config('app.otp_expire_time', 10));
        ...
        Log::info('OTP generated successfully', ['otp' => $otp, 'email' => $email, 'mobile' => $mobile, 'expires_at' => $expiresAt]);
        return [
            'success' => true,
            'otp' => $otp,
            'records' => $otpRecords,
