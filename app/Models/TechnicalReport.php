...
class TechnicalReport extends Model
{
    ...
    {
        return asset('storage/' . Config::get('file_paths')['TECHNICAL_REPORT_FILE_EN_PATH'] . '/' . $this->file_name);
    }

    public function getFileUrlHiAttribute()
    {
        return asset('storage/' . Config::get('file_paths')['TECHNICAL_REPORT_FILE_HI_PATH'] . '/' . $this->file_name_hi);
    }

    public function getFilePathEnAttribute()
    {
        return $this->file_name ? base64_encode(Config::get('file_paths')['TECHNICAL_REPORT_FILE_EN_PATH'] . '/' . $this->file_name) : null;
    }
    public function getFilePathHiAttribute()
    {
        return $this->file_name_hi ? base64_encode(Config::get('file_paths')['TECHNICAL_REPORT_FILE_HI_PATH'] . '/' . $this->file_name_hi) : null;
    }
    ...
    public function getActivitylogOptions(): LogOptions
    {
        return LogOptions::defaults()
            ->logAll()
            ->useLogName('technical_report')
            ->logOnlyDirty()
            ->setDescriptionForEvent(fn(string $eventName) => "Technical Report model has been {$eventName}");
    }
